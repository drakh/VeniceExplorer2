...
public class MarkerPlane extends BaseObject3D
{
	...
	public MarkerPlane(float width, float height, int segmentsW, int segmentsH)
	{
		this(width, height, 1, segmentsW, segmentsH);
	}

	public MarkerPlane(float width, float height, int direction, int segmentsW,
			int segmentsH)
	{
		super();
		mWidth = width;
	...
	private void init()
	{
		...
		for (i = 0; i <= mSegmentsW; ++i)
		{
			...
			{
				vertices[vertexCount] = ((float) i / (float) mSegmentsW - 0.5f)
						* mWidth;
				if (mDirection == 0)
				{
					vertices[vertexCount + 1] = 0;
					vertices[vertexCount + 2] = ((float) j / (float) mSegmentsH - 0.5f)
							* mHeight;
				}
				else
				{
					vertices[vertexCount + 1] = ((float) j / (float) mSegmentsH - 0.5f)
							* mHeight;
					vertices[vertexCount + 2] = 0;
				}

				textureCoords[texCoordCount++] = (float) j / (float) mSegmentsW;
				textureCoords[texCoordCount++] = 1f - (float) i
						/ (float) mSegmentsH;
